...
public partial class PostWKconfirm : System.Web.UI.Page
{
    string originalDataPath = @"\\CIERANT-TAPER\Clients\Horizon BCBS\14-0358_Post_Welcome_Kits\SECURE DATA\PROD_Medicare\SHIP_CONFIRMATION";
    ...
    protected void UploadFiles(object sender, EventArgs e)
    {
        ...
        dbU.ExecuteScalar("delete from Tempo_PWK_ship");
        ...
        Connection.Open();
         ...
         Connection.Close();
         string strSql = "select * from Tempo_PWK_ship left join HOR_MedicareMailData_Reports " +
                         "on Tempo_PWK_ship.recnum = HOR_MedicareMailData_Reports.recnum " +
                         "where HOR_MedicareMailData_Reports.recnum is null";
         DataTable tblErrors = new DataTable();
    ...
    protected void UpdateShip(object sender, EventArgs e)
    {
        ...
        GlobalVar.dbaseName = "BCBS_Horizon";
        dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
        string updateSQL = "update HOR_MedicareMailData_Reports " +
                            "set HOR_MedicareMailData_Reports.ShippedDate = Tempo_PWK_ship.Shipdate " +
                            "from Tempo_PWK_ship inner join HOR_MedicareMailData_Reports " +
                            "on Tempo_PWK_ship.recnum = HOR_MedicareMailData_Reports.recnum ";
        dbU.ExecuteNonQuery(updateSQL);
        
        
        string strSql = "select distinct FileName from Tempo_PWK_ship";
        ...
        foreach (DataRow dr in filesProcessed.Rows)
        {
            ...
            //move file
            File.Move(originalDataPath + "\\" + dr[0].ToString(), originalDataPath + "\\Processed\\" + dr[0].ToString());
        }



        strSql = "select Tempo_PWK_ship.* from Tempo_PWK_ship left join HOR_MedicareMailData_Reports " +
                        "on Tempo_PWK_ship.recnum = HOR_MedicareMailData_Reports.recnum " +
                        "where HOR_MedicareMailData_Reports.recnum is null";
        tblErrors = dbU.ExecuteDataTable(strSql);
        if (tblErrors.Rows.Count > 0)
        ...
        sendmail.SendMail("Post Welcome Kits Shipping Updates", "tkrompinger@apps.cierant.com,kcarpenter@apps.cierant.com, rchico@apps.cierant.com",
    ...
    public DataTable evaluate_XLSs(string fileName)
    {
        ...
        foreach (DataRow row in tmpqryTable.Rows)
        {
            ...
            row["Recnum"] = Regex.Replace(row["Recnum"].ToString(), "[^0-9.]", "");
        }
