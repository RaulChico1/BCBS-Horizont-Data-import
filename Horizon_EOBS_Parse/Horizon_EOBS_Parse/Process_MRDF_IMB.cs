...
namespace Horizon_EOBS_Parse
{
    public class Process_MRDF_IMB
    {
        ...
        public string Process_IMB(string filename, string locationLocal)
        {
            ...
            dataCSV.Columns.Add("DATETIME_MARKED");
            dataCSV.Columns.Add("DEST_DB");
            ...
            foreach (DataRow row in dataCSV.Rows) // Loop over the rows.
            {
                ...
                row["DATETIME_IMPORTED"] = fileInfo.CreationTime.ToShortDateString() + ' ' + fileInfo.CreationTime.ToShortTimeString();    //DateTime.Now.ToShortDateString() + ' ' + DateTime.Now.ToShortTimeString();
            ...
            Connection.Close();
            //}
            dbU.ExecuteNonQuery("delete from IMBDATA_IMPORT_tmp where ACCOUNT_NUMBER < 50000000");
            dbU.ExecuteNonQuery("delete from [USPS_IMBSCAN].[dbo].[IMBDATA_IMPORT] where IMPORT_FILENAME = '" + fileInfo.Name + "'");
            dbU.ExecuteNonQuery("upd_IMBDATA_IMPORT_to_USPS_IMBSCAN");
        ...
        public string Process_MRDF(string filename, string locationLocal)
        {
            ...
            if (!File.Exists(ProcessVars.gmappingFileMRDF))
                ...
                errorsParse = "Mapping file not found.";
        ...
        private List<List<Field>> ParseFile(string inputFile)
        {
            ...
            {
                errorsParse = ex.Message + " " + inputFile;
            }
        ...
        public string Upd_MRDF_to_ID_Cards(string filename)
        {
            GlobalVar.dbaseName = "BCBS_Horizon";
            dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
            dbU.ExecuteScalar("MRDF_Update_IDCards");    // table:  MRDF_ID_CARDS_Historical
