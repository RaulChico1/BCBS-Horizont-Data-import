        ...
        public string evaluate_TXT(string fileName, string secondValue, string LastWriteTime, string newSYSID)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                try
                {
                    ...
                    if (currLine > prevline && line.Length > 1 && fDJDE)
                    {
                        if (line.IndexOf("11") == 0 && fsys)
                        {
                            prevline = currLine;
                            headers.Add(line.Substring(3, line.Length - 3).TrimStart().TrimEnd());
                        }
                        if (line.IndexOf(" 3") == 0 && fsys)
                        {
                            prevline = currLine;
                            if (line.Length > 3)
            ...
            if (errorcount == 0)
            {
                if (sysout != "")
                {
                    if (DataTable.Rows.Count > 0)
                    {
                        ...
                        string resultcsv = create_cas__csv.create_Default_CAS_CSV(
                                            fileName, DataTable, "PND", Recnum, DataTable.Rows.Count.ToString(), sysout, jobID, LastWriteTime);
                        if (resultcsv != "")
                    ...
                    else
                    {
                        ...
                        //save with zero
                        create_cas__csv.update_w_errors_zero(fileName, "PND", "No recods in file");
                    }
                ...
                else
                {
                    ...
                    errorcount++;
                    //save with error
                    create_cas__csv.update_w_errors_zero(fileName, "PND", "No SYSOUT ID in file");
                }
            }
            else
            {
                ...
                //save with error
                create_cas__csv.update_w_errors_zero(fileName, "PND", "error count " + errorcount);
            }
