...
namespace Horizon_EOBS_Parse
{
    ...
    {
        public void CreateExcelFile(DataTable dataToTicket, string location, string ticketNO, string dateofProcess)
        {
            {
            ...
            anothernum:
                xx++;
            string docName = location + "Job Ticket_" + ticketNO + "_" + dateofProcess.Replace("-","_") + "__" + xx + ".xls";
            string FileNAME_network = @"\\Cierant-taper\clients\Horizon BCBS\NoticeLetters\JobTickets\" + "Job Ticket_" + ticketNO + "_" + dateofProcess.Replace("-", "_") + "__" + +xx + ".xlsx";
        ...
        public void CreateExcelFileTables(DataTable dataTo1, string Name1, string location, DataTable dataTo2, string Name2, DataTable dataTo3, string Name3)
        {
            {
                int totalRecs = dataTo1.Rows.Count;

                if (File.Exists(location))
                    File.Delete(location);

                // Create a Wordprocessing document. 
                using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(location, SpreadsheetDocumentType.Workbook))
                {
                    // Add a WorkbookPart to the document.
                    WorkbookPart workbookpart = spreadsheetDocument.AddWorkbookPart();
                    workbookpart.Workbook = new Workbook();
                    ...
                    cellVal = Name1; cellLoc = "A1";
                    SetCell(worksheetPart.Worksheet, cellVal, cellLoc);

                    for (int index = 0; index < dataTo1.Columns.Count; index++)
                    {
                        cellVal = dataTo1.Columns[index].ColumnName;
                        cellLoc = alpha[index].ToString() + xRow;
                        SetCell(worksheetPart.Worksheet, cellVal, cellLoc);
                        bkColumn = index;
                    }
                    ...
                    foreach (DataRow rowd in dataTo1.Rows)
                    {
                        for (int index = 0; index < dataTo1.Columns.Count; index++)
                        {
                            cellVal = rowd[index].ToString().Replace("\0", "");
                            cellLoc = alpha[index].ToString() + xRow;
                            SetCell(worksheetPart.Worksheet, cellVal, cellLoc);
                        }
                        xRow++;
                    ...
                    xRow++;
                    xRow++;
                    cellVal = Name2; cellLoc = "A" + xRow.ToString();
                    //xRow++;
                    SetCell(worksheetPart.Worksheet, cellVal, cellLoc);

                    for (int index = 0; index < dataTo2.Columns.Count; index++)
                    {
                        cellVal = dataTo2.Columns[index].ColumnName;
                        cellLoc = alpha[index].ToString() + xRow;
                        SetCell(worksheetPart.Worksheet, cellVal, cellLoc);
                        bkColumn = index;
                    }
                    ...
                    foreach (DataRow rowd in dataTo2.Rows)
                    {
                        for (int index = 0; index < dataTo2.Columns.Count; index++)
                        {
                            cellVal = rowd[index].ToString().Replace("\0", "");
                    ...
                    cellVal = Name3; cellLoc = "A" + xRow.ToString();
                    xRow++;
                    SetCell(worksheetPart.Worksheet, cellVal, cellLoc);

                    for (int index = 0; index < dataTo3.Columns.Count; index++)
                    {
                        cellVal = dataTo3.Columns[index].ColumnName;
                        cellLoc = alpha[index].ToString() + xRow;
                        SetCell(worksheetPart.Worksheet, cellVal, cellLoc);
                        bkColumn = index;
                    }
                    ...
                    xRow++;
                    foreach (DataRow rowd in dataTo3.Rows)
                    {
                        for (int index = 0; index < dataTo3.Columns.Count; index++)
                        {
                            cellVal = rowd[index].ToString().Replace("\0", "");
