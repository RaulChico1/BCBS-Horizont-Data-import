...
//            string zipName = ProcessVars.InputDirectory + @"FromCASS\" + gName + "-" + GlobalVar.DateofProcess.AddDays(-2).ToString("yyyyMMdd") + ".zip";
        ...
        public void zipF10_FIM(string zipname, string cvsfilename, FileInfo file)
        {
            try
            {
                using (ZipArchive newFile = ZipFile.Open(zipname, ZipArchiveMode.Update))
                {
                    newFile.CreateEntryFromFile(cvsfilename, file.Name.Replace(".txt", ".csv"));
                    newFile.CreateEntryFromFile(cvsfilename.Replace(".csv", ".pdf"), file.Name.Replace(".txt", ".pdf"));
                    newFile.CreateEntryFromFile(cvsfilename.Replace(".csv", ".txt"), file.Name);
                }
            }
            catch (Exception ex)
            {
                var result2 = ex.Message.ToString();

            }
            
        }
        public void zipAR06(string zipname, string filename, string justname)
        {
            try
            {
                using (ZipArchive newFile = ZipFile.Open(zipname, ZipArchiveMode.Update))
                {
                    newFile.CreateEntryFromFile(filename, justname);
                }
            }
            catch (Exception ex)
            {
