...
namespace Horizon_EOBS_Parse
{
    public class Notice_Letter
    {
        ...
        public void processNotices(string filename, string dateFile,string newSYSID)
        {
                    ...
                    || file.Name.ToUpper().IndexOf("EP005204") == 0)
            {
                try
                {
                    ...
                    string error = evaluate_TXT(file.FullName, dateFile,newSYSID);
                    if (error != "")
                        errors = errors + error + "\n\n";
                }
                catch (Exception ez)
            ...
            if (file.Name.IndexOf("EP005303") == 0)
            {
                ...
                {
                    //check jobid definition
                    string error = evaluate_TXT503(file.FullName, dateFile,newSYSID);
                    if (error != "")
                        errors = errors + error + "\n\n";
                }
                catch (Exception ez)
            ...
            if (file.Name.IndexOf("EP006003") == 0)
            {
                ...
                {
                    //check jobid definition
                    string error = evaluate_TXT603(file.FullName, dateFile,newSYSID);
                    if (error != "")
                        errors = errors + error + "\n\n";
                }
                catch (Exception ez)
        ...
        public string evaluate_TXT(string fileName, string dateFile,string newSYSID)
        {
            ...
            if (sysout != "")
            {
                ...
                string moveName = ProcessVars.InputDirectory + fileInfo.Name.Substring(0, fileInfo.Name.Length - 4) + "_process_" + GlobalVar.DateofProcess.ToString("yyyy_MM_dd_HH_mm") + ".csv"; 
                File.Copy(pName, moveName);
        ...
        public string evaluate_TXT503(string fileName, string dateFile, string newSYSID)
        {
            GlobalVar.dbaseName = "BCBS_Horizon";
            dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
        ...
        public string evaluate_TXT603(string fileName, string dateFile,string newSYSID)
        {
            //string str = ConfigurationManager.ConnectionStrings["conStrProd"].ToString();
            ...
            if (sysout != "")
            {
                ...
                if (errors == 0)
                {
                    ...
                    dbU.ExecuteScalar("delete from HOR_parse_TMP_6003_PastDueNotice");
                    dbU.ExecuteScalar("Insert into HOR_parse_SEQ (Recnum, TableName, datetime) values(" + (GRecnum - 1) + ",'HOR_parse_6003_PastDueNotice', GETDATE())");
        ...
        public void produceSummary( )
        {
            ...
            if (processedData.Rows.Count > 0)
            {
                ...
                sendmail.SendMail("PastDue Notice Upload", "tkrompinger@apps.cierant.com, kcarpenter@apps.cierant.com,dgannuscio@apps.cierant.com,kmcnamara@apps.cierant.com,cgaytan@apps.cierant.com,rchico@apps.cierant.com",
