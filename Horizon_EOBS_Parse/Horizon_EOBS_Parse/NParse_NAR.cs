...
namespace Horizon_EOBS_Parse
{
    public class NParse_NAR
    {
        ...
        public string evaluate_TXT(string fileName, string secondValue, string LastWriteTime,string newSYSID)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                try
                {
                    ...
                    //}
                    if (currLine > prevline && line.Length > 1)
                    {
                        if (line.IndexOf("1 ") == 0)
                        {
                            prevline = currLine;
                            ...
                            fDate = true;
                            docDate = line.Substring(2, 49).TrimStart().TrimEnd();
                        }

                        if (currLine > prevline && line.IndexOf("0 ") == 0 && line.IndexOf(addr2) == -1)
                        {
                            prevline = currLine;
                            fsAddrs = true;
                            fDate = false;
                            addrs.Add(line.Substring(2, 49).TrimStart().TrimEnd());
                            countinAddr++;
                        }

                        if (currLine > prevline && line.Substring(0, 2) == "  " && fsAddrs)
                        {
                            if (line.Length > 1)
                            {
                                if (line.Length < 52)
                                {
                                    ...
                                    string tmpLine = line.Substring(2, line.Length - 2).TrimStart().TrimEnd();
                                    while (tmpLine.Contains("  ")) tmpLine = tmpLine.Replace("  ", " ");
                                    addrs.Add(tmpLine);
                                }
                                else
                                {
                                    if (line.Substring(0, 49).TrimStart() != "")
                                    {
                                        prevline = currLine;
                                        countinAddr++;
                                        string tmpLine = line.Substring(2, 49).TrimStart().TrimEnd();
                                        while (tmpLine.Contains("  ")) tmpLine = tmpLine.Replace("  ", " ");
                                        addrs.Add(tmpLine);
                                    }
                                    else
                    ...
                    if (currLine > prevline && fsEndAddr)
                    {
                        while (countinAddr < 6)
                        {
                            countinAddr++;
            ...
            if (errorcount == 0)
            {
                if (sysout != "")
                {
                    if (DataTable.Rows.Count > 0)
                    {
                        ...
                        string resultcsv = create_cas__csv.create_Default_CAS_CSV(
                                            fileName, DataTable, "NAR", Recnum, (DataTable.Rows.Count * 2).ToString(), sysout, jobID, LastWriteTime);
                        if (resultcsv != "")
                    ...
                    else
                    {
                        ...
                        //save with zero
                        create_cas__csv.update_w_errors_zero(fileName, "NAR", "No recods in file");
                    }
                ...
                else
                {
                    ...
                    errorcount++;
                    //save with error
                    create_cas__csv.update_w_errors_zero(fileName, "NAR", "No SYSOUT ID in file");
                }
            }
            else
            {
                ...
                //save with error
                create_cas__csv.update_w_errors_zero(fileName, "NAR", "error count " + errorcount);
            }
        ...
        public void addToTable()
        {
            ...
            row["Field2"] = docDate.TrimStart().TrimEnd();
            row["Field3"] = currLine;
            row["Field4"] = Feed;
            row["Addr1"] = addrs[0];
            row["Addr2"] = addrs[1];
