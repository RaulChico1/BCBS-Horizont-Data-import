...
namespace Horizon_EOBS_Parse
{
    public class NParse_EPA_EPM
    {
        ...
        string inaddr = "";
        string FileClass = "";
        string sysout, psysout, jobname, jobID, pDate, aDate, seqNum, scount, mailStop, checknumber, Feed, docDate, LetterProduced, idNumber, Dollrs, issueDate, medFLAG, UCode;
        string errors = "";
        int errorcount = 0;
        int currLine = 0;
        DBUtility dbU;
        ...
        public string evaluate_TXT(string fileName, string secondValue, string LastWriteTime,string newSYSID)
        {
            ...
            sysout = psysout = jobname = pDate = jobID = aDate = seqNum = scount = mailStop = checknumber = Feed = docDate = LetterProduced = idNumber = issueDate = Dollrs = medFLAG = UCode = string.Empty;
                ...
                try
                {
                    ...
                    if (line.IndexOf(addr1) == 0 && fsys)
                    {
                        if (PrevValLine.Length > 1)
                        {
                            ...
                            idNumber = words[2];
                            UCode = words[3];
                            issueDate = words[4] + "/" + words[5] + "/" + words[6];
                            Dollrs = words[7];
                        }
                        countinAddr = 0;
                        if (line.Substring(3, line.Length - 3).TrimStart().TrimEnd() != "MED ADV")
                            addrs.Add(line.Substring(3, line.Length - 3).TrimStart().TrimEnd());     //was MED ADV
                        prevline = currLine;
                        inAddrs = true;
                        inaddr = "";
                    }
