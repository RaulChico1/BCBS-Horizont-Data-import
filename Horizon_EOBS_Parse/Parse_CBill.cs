...
{
    public class Parse_CBill
    {
        DataTable C_BILLS = EOBs_Table();
        List<string> addrs = new List<string>();
        int Recnum = 1;
        int C_Recnum = 1;
        int countinAddr = 0;
        string sysout, jobname, pDate, aDate, seqNum, scount, mailStop, JobID, fileNAME;
        string errors = "";
        int errorcount = 0;
        DBUtility dbU;
        ...
        public string zipFilesinDir(string dateProcess)
        {
            ...
            {
                DirectoryInfo originalPDFs = new DirectoryInfo(ProcessVars.CBInputDirectory);
                ...
                foreach (FileInfo file in FilesPDF)
                {
                    if (file.Name.IndexOf("DISPATCH") == -1)
                    {
                        try
                        {
                            string error = evaluate_TXT(file.FullName, file.Name);
                            if (error != "")
                                errors = errors + error + "\n\n";
                        }
                        catch (Exception ez)
        ...
        public string Process_Cbills(string filename, string dateFile)
        {
            string error = evaluate_TXT(filename,dateFile);
            return error;
        }
        public string evaluate_TXT(string fileName, string dateFile)
        {
            ...
            string seqNN = "SEQ # ";
            string add1 = " 2";
            sysout = jobname = pDate = aDate = seqNum = scount = mailStop = JobID = fileNAME = string.Empty;
            ...
            if (fileInfo.Name.IndexOf("EPBM") == 0)
                isMedicare = true;
                ...
                try
                {
                    ...
                    if (line.IndexOf(seqNN) != -1 && fsys)
                    {
                       // mailStop = line.Substring(4, line.Length - 10);
                        mailStop = searchTextSTART(seqNN, line, currLine);
                        seqNum = searchText2(seqNN, line, currLine);
                        fsCount = true;
                        fSeq = true;
                    }
        
                    if (line.IndexOf(add1) == 0 && fsCount && countinAddr < 7 && !fSeq)
                    {
                        countinAddr++;
                        addrs.Add(line.Substring(add1.Length, line.Length - add1.Length).Replace("....","").Trim());
                        
                    }

                    fSeq = false;

                    if (line.IndexOf(add1) == 0 && fsCount && countinAddr == 7)
                    {
                        addToTable(isMedicare);
                        fsCount = false;
                    }
            ...
            file.Close();
            DataTable working_C_BILLS = C_BILLS.Clone();
            //move addrs up
            for (int i =0; i < C_BILLS.Rows.Count; i++)
            {
                var stringArr = C_BILLS.Rows[i].ItemArray.Select(x => x.ToString()).ToList();
                for (int ii = 16; ii < stringArr.Count; ii++)
                {
                    if (stringArr[ii].ToString() == "" || stringArr[ii].ToString() == "____")
                    {
                        stringArr.RemoveAt(ii);
                        ii--;
                    }
                }
                DataRow nrow = working_C_BILLS.NewRow();
                int ind = 0;
                ...
                working_C_BILLS.Rows.Add(nrow);
            }
            foreach (DataRow row in working_C_BILLS.Rows)
            {
                for (int ii = 22; ii > 0; ii--)
                {
                    //if (row[16].ToString() == "A L MEDINA")
                    //    Recnum = Recnum;
                    if (row[ii].ToString() != "")
                    {
                        row[22] = row[ii];
            ...
            if (errorcount == 0)
            {
                if (sysout != "")
                {
                    if (working_C_BILLS.Rows.Count > 0)
                    {

                        string resultcsv = create_cas__csv.create_CBills_CAS_CSV(
                                            fileName, working_C_BILLS, "CBill_PastDueNotice", Recnum, C_BILLS.Rows.Count.ToString(), sysout, JobID, dateFile);
                        if (resultcsv != "")
                    ...
                    else
                    {
                        ...
                        //save with zero
                        create_cas__csv.update_w_errors_zero(fileName, "CBill_PastDueNotice", "No recods in file");
                    }
                ...
                else
                {
                    ...
                    errorcount++;
                    //save with error
                    create_cas__csv.update_w_errors_zero(fileName, "CBill_PastDueNotice", "No SYSOUT ID in file");
                }
            }
            else
            {
                ...
                //save with error
                create_cas__csv.update_w_errors_zero(fileName, "CBill_PastDueNotice", "error count " + errorcount);
            }
...
#region oldCode
               ...
               // {
               //     var stringArr = C_BILLS.Rows[i].ItemArray.Select(x => x.ToString()).ToList();
               //     for (int ii = 15; ii < stringArr.Count; ii++)
               //     {
               //         if (stringArr[ii].ToString() == "" || stringArr[ii].ToString() == "____")
               //         {
               //             stringArr.RemoveAt(ii);
               ...
               //     }
               // }
               // DataTable working_c_BILLS2 = C_BILLS.Copy();
               // working_c_BILLS2.Columns.Remove("MED_Flag");
               // string directoryAfterCass = ProcessVars.InputDirectory + "FromCASS";
               // //string directoryAfterCass = ProcessVars.GBOutputDirectory + "FromCASS";
               ...
               // string pName = ProcessVars.CBOutputDirectory + pNameToCASS;
               // //string pName = ProcessVars.OutputDirectory + sysout + "_" + DateTime.Now.ToString("yyyy_MM_dd_HH_mm") + ".csv";
               // if (File.Exists(pName))
               //     File.Delete(pName);
               // var fieldnames = new List<string>();
               // for (int index = 0; index < working_c_BILLS2.Columns.Count; index++)
               // {
               //     fieldnames.Add(working_c_BILLS2.Columns[index].ColumnName);
               ...
               // // add to dbase
               // //working_C_BILLS.Columns.Remove("Med_Flag");
               // string colnames = "";
               // for (int index = 0; index < working_C_BILLS.Columns.Count; index++)
               // {
               //     string colname = working_C_BILLS.Columns[index].ColumnName;
               //     colnames = colnames + ", [" + colname + "]";
               // }

               // GlobalVar.dbaseName = "BCBS_Horizon";
               // dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
               // dbU.ExecuteScalar("delete from HOR_parse_TMP_CBill_PastDueNotice");
                
               // int errors = 0;
               // string recnumError = "";
               // string insertCommand1 = "Insert into HOR_parse_TMP_CBill_PastDueNotice([FileName],[ImportDate]" + colnames + ") VALUES ('";
               // foreach (DataRow row in working_C_BILLS.Rows)
               // {
               ...
               //     int totrecs = working_C_BILLS.Rows.Count;
               //     dbU.ExecuteScalar("Insert into HOR_parse_CBill_PastDueNotice select * from HOR_parse_TMP_CBill_PastDueNotice");
               //     dbU.ExecuteScalar("delete from HOR_parse_TMP_CBill_PastDueNotice");
               ...
               //      if (FileCount == 0)
               //      {
               //          //dbU.ExecuteScalar("Insert into HOR_parse_files_to_CASS(RecordsNum, FileNameCASS, FileName, SentDate, TableName) values(" +
               //          //                   totrecs + ",'" + pNameToCASS + "','" + fileInfo.Name + "', GETDATE(),'HOR_parse_CBill_PastDueNotice')");
               ...
               //            totrecs + ",'" + totrecs + "','" + fileInfo.Name + "_File not to CASS_" + DateTime.Now.ToString("yyyy_MM_dd_HH_mm") + "','" + fileInfo.Name + "','" + dateFile + "','HOR_parse_CBill_PastDueNotice" + "','" + directoryAfterCass + "','" + sysout + "','" + jobname + "','Receive','Y')");

               //      }
               //      else
               //      {
               //          dbU.ExecuteScalar("Update HOR_parse_files_to_CASS set RecordsNum =" +
               //                             totrecs + ", SentDate = GETDATE(), TableName = 'HOR_parse_CBill_PastDueNotice', Processed = NULL where FileNameCASS = '" + pNameToCASS + "'");

               //      }
               //      dbU.ExecuteScalar("Insert into HOR_parse_SEQ (Recnum, TableName, datetime) values(" + (Recnum - 1) + ",'HOR_parse_CBill_PastDueNotice', GETDATE())");
               // }
               // outputSCI(fileInfo.Name, "HOR_parse_CBill_PastDueNotice", directoryAfterCass);
            #endregion
        }
        public void outputSCI(string FFName, string TTName, string directoryAfterCass)
        {
            ...
            DataTable datato_SCI = dbU.ExecuteDataTable("HOR_rpt_PARSE_cbILLSto_SCI", sqlParams);
        ...
        public void addToTable(bool isMedicare)
        {
            var row = C_BILLS.NewRow();
            row["Sysout"] = sysout;
            ...
            if (isMedicare)
                ...
                row["MED_Flag"] = "N";
            row["JOBID"] = JobID;
            row["Addr1"] = addrs[0].Replace("_",""). Trim();
            row["Addr2"] = addrs[1].Trim();
            row["Addr3"] = addrs[2].Trim();
            row["Addr4"] = addrs[3].Trim();
            row["Addr5"] = addrs[4].Trim();
            row["Addr6"] = addrs[5].Trim();
            
            //row["fileName"] = fileNAME;
            C_BILLS.Rows.Add(row);
            addrs.Clear();
            countinAddr = 0;
            Recnum++;
            C_Recnum++;
