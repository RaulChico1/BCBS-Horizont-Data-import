    ...
    public class Parse_Inv_pdf
    {
                    ...
                    if (words[0].ToString().IndexOf("$$METADATA$$") != -1)
                    {
                        ...
                        for (int i = 1; i < words.Length; i++)
                        {
                            ...
                            if (tmp.Contains("SEQ #"))
                            {
                                ...
                                if (posc + 6 > tmp.Length)
                                    m_transID = tmp.Substring(posc, tmp.Length - posc);
                                else
        ...
        public string finalprocessInv(string direcTory, string dateHLGS, string unzipName, string cycleDate, string FileType, DateTime lastW)
        {
            ...
            if (unzipName.IndexOf("INVCON") == -1)
            {
                ...
                spName = "HOR_rpt_PARSE_Non_Inv_noBCC_to_SCI";
            }
            else
            {
                spName = "HOR_rpt_PARSE_Inv_noBCC_to_SCI";
            }
            //upload to sql
            ...
            sqlParams = null;
            sqlParams = new SqlParameter[] { new SqlParameter("@FileName", unzipName), new SqlParameter("@table", "HOR_parse_" + FileType) };

            
            DataTable datato_SCI = dbU.ExecuteDataTable(spName, sqlParams);
            int totrecs = 0;
            string fileNameCass = unzipName.Replace(".pdf", ".pdf  File not to CASS");
            ...
            File.Copy(pName, ProcessVars.OtherProcessed + unzipName.Replace(".pdf", "") + ".csv",true);
            ...
            //string ResultsBack_CASS_S = processRedturnsS.ProcessFiles(FileType);    //"SBC"
        ...
        private static DataTable pdfs_Table_MAPDP()
        {
            DataTable newt = new DataTable();
            newt.Clear();
            newt.Columns.Add("Recnum");
        ...
        public string zipFilesinDir_MAEOB(string dateProcess, string directory)
        {

            if (Directory.Exists(directory))
            {
                DirectoryInfo originalPDFs = new DirectoryInfo(directory);
                FileInfo[] FilesPDF = originalPDFs.GetFiles("MAEOB*.pdf");
                if (FilesPDF.Count() > 0)
                {
                    GlobalVar.dbaseName = "BCBS_Horizon";
                    dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
                    ...
                    foreach (FileInfo file in FilesPDF)
                    {
                        if (file.Name.Substring(0, 1) != "_")
                        {
                            ...
                            {
                                //HOR_parse_CareRadius_2
                                var fileU = dbU.ExecuteScalar("select FileName from HOR_parse_MAEOB where FileName = '" + file.Name + "'");
                                //+ "' and CONVERT(DATE,ImportDate_Start)=CONVERT(DATE,GETDATE()
                                if (fileU != null)
                                {
                                    dbU.ExecuteScalar("delete from HOR_parse_MAEOB where FileName = '" + file.Name + "'");
                                    dbU.ExecuteScalar("delete from HOR_parse_files_to_CASS where FileName = '" + file.Name + "'");
                                }

                                invPdfs.Clear();
                                string error = evaluate_MAEOB(file.FullName, "");
                                if (error != "")
                                    errors = errors + error + "\n\n";
                                else
                                    if (invPdfs.Rows.Count > 0)
                                    {
                                       ...
                                       //NO updating BackCASS from bcc???????

                                        finalprocessInv(directory.Replace("\\from_FTP", ""), dateProcess, file.Name, dateProcess, "MAEOB", dateUpload);   //    finalprocess(direcTory, dateHLGS, "Coba", cycleDate, "HLGS");
                                        File.Copy(file.FullName, ProcessVars.OtherProcessed + file.Name,true);
                                        File.Move(file.FullName, file.FullName.Replace("MAEOB", "__MAEOB"));
        ...
        public string evaluate_MAEOB(string fileName, string dest)
        {
                ...
                for (int page = 1; page <= reader.NumberOfPages; page++)
                {
                    ...
                    if (words[0].ToString().IndexOf("$$METADATA$$") != -1)
                    {
                        ...
                        for (int i = 1; i < words.Length; i++)
                        {
                            ...
                            if (tmp.Contains("NEWARK, NJ 07101-0820"))
                            {
                                boolYearFiled = true;
                                addrs.Clear();
                            ...
                            if (boolYearFiled)
                            {
                                ...
                                if (tmp.Contains("IMPORTANT PLAN INFORMATION"))
                                {
                                    //addrs.Add(words[i].ToString().TrimStart().TrimEnd());
                                    while (addrs.Count < 5)
                                    {
                                    ...
                                    addToTableInv(1, fileInfo.Name, "MAEOB");
                                    boolYearFiled = false;
                                    endAddr = true;
                                }
                                else
                    ...
                    else
                    {
                        ...
                        if (m_IDNumber == "")
                        {
                            ...
                            for (int i = 1; i < words.Length; i++)
                            {
                                tmp2 = words[i].ToString().ToUpper();
                                if (tmp2.Contains("3HZ"))
                                {
                                    int posc1 = tmp2.IndexOf("3HZ");
                                    m_IDNumber = tmp2.Substring(posc1, 12);
                                    invPdfs.Rows[invPdfs.Rows.Count - 1]["artifactId"] = m_IDNumber;
                                    break;
                                }
                            }
        ...
        public DataTable evaluate_IndividualPdf(string fileName)
        {
            FileInfo fileInfo = new System.IO.FileInfo(fileName);
            DataTable data_o = new DataTable();
            data_o.Columns.Add("pdfName");
            data_o.Columns.Add("pdfPages");
            data_o.Columns.Add("pdfAccount");
            data_o.Columns.Add("pdfInvoice");
            PdfReader reader = new PdfReader(fileName);
            totP = reader.NumberOfPages;
            string Invoice = "";
            string Account = "";
            for (int page = 1; page <= reader.NumberOfPages; page++)
            {
                ...
                for (int i = 1; i < words.Length; i++)
                {
                    if (words[i].ToString().IndexOf("INVOICE") != -1 && words[i].ToString().IndexOf("ACCOUNT") != -1)
                    {
                        int poscInv = words[i].ToString().IndexOf("INVOICE");
                        Account = words[i].ToString().Substring(0, poscInv - 1).Replace("ACCOUNT#:", "").Trim();
                        Invoice = words[i].ToString().Substring(poscInv - 1, words[i].ToString().Length - poscInv).Replace("INVOICE#:", "").Trim();

                        var row = data_o.NewRow();
                        row["pdfName"] = fileInfo.Name;
                        row["pdfPages"] = totP.ToString();
                        row["pdfAccount"] = Account; // words[0].ToString().Substring(0, poscInv - 1).Trim();
                        row["pdfInvoice"] = Invoice; // words[0].ToString().Substring(poscInv - 1, words[0].ToString().Length - poscInv).Trim();
                        data_o.Rows.Add(row);
                        break;
                    }
        ...
        public int evaluate_IndividualPdfSimple(string fileName)
        {
            int totPages = 0;
