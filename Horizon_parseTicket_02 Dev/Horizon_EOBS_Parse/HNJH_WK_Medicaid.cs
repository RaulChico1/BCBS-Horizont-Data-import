...
namespace Horizon_EOBS_Parse
{
    public class HNJH_WK_Medicaid
    {
        ...
        public string Process_WK_Medicare(string filename, string locationLocal)
        {
            #region main_Process
            int updErrors = 0;
            string errors = "";
            ...
            foreach (DataRow rowU in sortedData.Rows)
            {
                ...
                {
                    if (rowU["MEME_MCTR_LANG"] == "SPAN")
                        rowU["Translate1"] = resultQ + "S";
                    else
                ...
                {
                    if (rowU["MCTR_Desc"].ToString() != "")
                    {
                        rowU["DL"] = "N";
                    }
                    if (wMCTR_Desc != "Welcome Kit" && rowU["CSPI_Desc"].ToString() != "")
                    {
            ...
            string strsql2 = "update  HOR_parse_HNJH_WK set dl = 'N' where filename = '" + fileInfo.Name + "' and len(UpdCounty) < 2 ";
            dbU.ExecuteNonQuery(strsql2);
            strsql2 = "update  HOR_parse_HNJH_WK set dl = 'N' where filename = '" + fileInfo.Name + "' and len(MCTR_DESC) < 1";
            dbU.ExecuteNonQuery(strsql2);
            //update to DL = N 
        ...
        public string ProcessBackData()
        {
            ...
            strsql = "select count(*) from  HOR_parse_files_to_CASS where TableName = 'HOR_parse_HNJH_WK' and Processed is null and convert(date,importdate) = '" + DateTime.Now.ToString("yyyy-MM-dd") + "'";
            int totrecsNP = Int32.Parse(dbU.ExecuteScalar(strsql).ToString());
            if (totrecsNP > 0)
                result = "file not returning from bcc " + totrecsNP.ToString();
            return result;
        }
        public string Print_HNJH_WK(string location, string locationLocal, string dateReport)
        {
            ...
            strsqlError = "SELECT * FROM HOR_parse_HNJH_WK where UpdState <> 'NJ' and convert(date,importdate) = '" + dateReport.Replace("_", "/") + "'";
            ...
            dbU.ExecuteNonQuery("update HOR_parse_HNJH_WK set DL = 'O'  where UpdState <> 'NJ'  and convert(date,importdate) = '" + dateReport.Replace("_", "/") + "'");
...
#region testing
            ...
            DataTable Xmpi_1_2_3 = dbU.ExecuteDataTable("select UpdCounty,Translate1, Translate2, Translate3, UpdAddr1, UpdAddr2, UpdAddr5, UpdAddr4,UpdCity, UpdState, UpdZip, Recnum  from HNJH_WKits_NJH_XMpie_all3 order by UpdCounty, Translate1, Translate2, Translate3, UpdAddr1");
            string LabelsCountys = @"\HNJH_WKits_County Data__" + dateReport + ".csv";
            createcsv.printCSV_fullProcess(TTDir + LabelsCountys, Xmpi_1_2_3, "", "");
            ...
            DataTable Xmpi_1_2_3Other = dbU.ExecuteDataTable("select UpdCounty,Translate1, Translate2, Translate3, UpdAddr1, UpdAddr2, UpdAddr5, UpdAddr4,UpdCity, UpdState, UpdZip, Recnum  from HNJH_WK_labels_Xmpie order by UpdCounty, Translate1, Translate2, Translate3, UpdAddr1");
            string LabelsCountysOther = @"\HNJH_WKits_County Data_Others_" + dateReport + ".csv";
            createcsv.printCSV_fullProcess(TTDir + LabelsCountysOther, Xmpi_1_2_3Other, "", "");
        ...
        public string Print_HNJH_WK_Medicare(string location, string locationLocal, string dateReport)
        {
            string result = "";
            appSets appsets = new appSets();
            appsets.setVars();
            ...
            strsqlError = "SELECT * FROM HOR_parse_HNJH_WK where UpdState <> 'NJ' and Translate1 in ('NJH-01','NJH-04','NJH-05') and convert(date,importdate) = '" + dateReport.Replace("_", "/") + "'";
            DataTable OutState = dbU.ExecuteDataTable(strsqlError);
            ferrorname = location + @"\HNJH_WKits_OUT_OF_STATE_" + dateReport + ".csv";
            createcsv.printCSV_fullProcess(ferrorname, OutState, "", "N");
            ...
            sqlParamsXMpie = null;
            sqlParamsXMpie = new SqlParameter[] { new SqlParameter("@Date", dateReport.Replace("_", "/")) };
            dbU.ExecuteScalar("HOR_rpt_HNJH_WK__XMpie_Date_hnjh_04_05", sqlParamsXMpie);



            SqlParameter[] sqlParamsLabels;
            sqlParamsLabels = null;
            sqlParamsLabels = new SqlParameter[] { new SqlParameter("@Translate", "NJH-04") };
            string LabelsFFName = @"\HNJH_WKits_NJH-04_Labels__" + dateReport + ".csv";
            DataTable NJH04_Labels = dbU.ExecuteDataTable("HOR_rpt_HNJH_WK__NJH04_Labels", sqlParamsLabels);
            createcsv.printCSV_fullProcess(TTDir + LabelsFFName, NJH04_Labels, "", "");


            sqlParamsLabels = null;
            sqlParamsLabels = new SqlParameter[] { new SqlParameter("@Translate", "NJH-05") };
            LabelsFFName = @"\HNJH_WKits_NJH-05_Labels__" + dateReport + ".csv";
            DataTable NJH05_Labels = dbU.ExecuteDataTable("HOR_rpt_HNJH_WK__NJH05_Labels", sqlParamsLabels);
            createcsv.printCSV_fullProcess(TTDir + LabelsFFName, NJH05_Labels, "", "");
