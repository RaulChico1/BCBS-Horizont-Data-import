...
namespace Horizon_EOBS_Parse
{
    ...
    {
        private static readonly string Password = ProcessVars.gPassword;
        public static readonly string PublicKey = ProcessVars.gPublicKey;
        //public static readonly string PublicKeyPath = IoHelper.BasePath + @"\" + PublicKey;
        public static readonly string PublicKeyPath = ProcessVars.gProgramPath + @"\" + PublicKey;
        ...
        private static readonly string PrivateKeyOnlyPath = ProcessVars.gProgramPath + @"\" + PrivateKeyOnly;
        ...
        public static void EncryptPgpFile(string inputFile, string outputFile, string publicKeyFile, bool armor, bool withIntegrityCheck)
        {
            ...
            MemoryStream outputBytes = null;
            PgpEncryptedDataGenerator dataGenerator = null;
            PgpCompressedDataGenerator dataCompressor = null;
            try
            {
                using (publicKeyStream = File.OpenRead(publicKeyFile))
                {
                    PgpPublicKey pubKey = ReadPublicKey(publicKeyStream);

                    using ( outputBytes = new MemoryStream())
                    {
                        dataCompressor = new PgpCompressedDataGenerator(CompressionAlgorithmTag.Zip);
                        PgpUtilities.WriteFileToLiteralData(dataCompressor.Open(outputBytes), PgpLiteralData.Binary, new FileInfo(inputFile));

                        dataCompressor.Close();
                         dataGenerator = new PgpEncryptedDataGenerator(SymmetricKeyAlgorithmTag.Cast5, withIntegrityCheck, new SecureRandom());
                        ...
                        using ( outputStream = File.Create(outputFile))
                        {
                            if (armor)
                            {
                                using ( armoredStream = new ArmoredOutputStream(outputStream))
                                {
