...
namespace Horizon_EOBS_Parse
{
    public class HNJH_To_XMPie
    {
        ...
        public string SentTo_XMpieTables(string PagNoXmpie, string fileName)
        {
            ...
            dbU.ExecuteScalar("delete Provider_Panel_2017");
            dbU.ExecuteScalar("delete HNJH_RosterProvider_Xmpie");
            //dbU.ExecuteScalar("delete HNJH_RosterProvider_Xmpie_new");


            dbU.ExecuteScalar("insert into [Provider_Panel_2017] select Pa.* from HOR_parse_HNJH_Panel_Roster_Panel Pa " +
                                "inner join HOR_parse_HNJH_Panel_Roster_Provider Pv " +
                                "on pv.Recnum = Pa.Recnum " +
                                "where Pv.PagNoXmpie = '" + PagNoXmpie + 
                                "' and Pv.filename = '" + fileName + "' order by PA.Network, PA.ProvGroupID, ProviderName, MemberName");   //and pv.CountofMembersonPanel > 20 and pv.CountofMembersonPanel < 35

            dbU.ExecuteNonQuery("update [Provider_Panel_2017] set TPLD = '' where TPLD = 'No'");
            dbU.ExecuteNonQuery("update [Provider_Panel_2017] set NewMember = 'Yes' where NewMember = 'Y'");
            dbU.ExecuteNonQuery("update [Provider_Panel_2017] set NewMember = '' where NewMember = 'N'");

            DataTable ProviderPanel = dbU.ExecuteDataTable("select * from [Provider_Panel_2017] order by ProvGroupID, ProviderName,MemberName");
        ...
        public string SentTo_XMpieTablesRecnum(int recnum, string fname)
        {
            string localdir = @"C:\CierantProjects_dataLocal\Horizon_Parse\DailyFiles\Roster\";
            string result = "";
            GlobalVar.dbaseName = "BCBS_Horizon";
            ...
            try
            {
                dbU.ExecuteScalar("insert into [Provider_Panel_2017] select Pa.* from HOR_parse_HNJH_Panel_Roster_Panel Pa " +
                                    "inner join HOR_parse_HNJH_Panel_Roster_Provider Pv " +
                                    "on pv.Recnum = Pa.Recnum " +
                                    "where Pv.Recnum = " + recnum + " order by ProviderName, Membername");   //and pv.CountofMembersonPanel > 20 and pv.CountofMembersonPanel < 35
            }
            catch (Exception ez)
            {
                result = result + recnum + "  " + ez.Message + "\n\n";
            }

            SqlParameter[] sqlParamsXmpie;
            sqlParamsXmpie = null;
            sqlParamsXmpie = new SqlParameter[] { new SqlParameter("@Recnum", recnum)};
            dbU.ExecuteScalar("HOR_upd_Provider_Xmpie_Recnum", sqlParamsXmpie);
            DataTable Provider = dbU.ExecuteDataTable("select * from HNJH_RosterProvider_Xmpie");
            createCSV createcsv = new createCSV();
           // string Networkfname = ProcessVars.SourceDataRoster + "\\Missing_Provider_" + recnum.ToString() + ".csv";
            string localname = fname + "Missing_Provider_" + recnum.ToString() + ".csv";
            bool resultCSV = createcsv.printCSV_fullProcess(localname, Provider, "", "Y");
            //if (File.Exists(Networkfname))
            //    File.Delete(Networkfname);
            //File.Copy(localname, Networkfname);
            ...
            DataTable panel = dbU.ExecuteDataTable("select * from Provider_Panel_2017  order by ProviderName, Membername");
            int totrecs = panel.Rows.Count; int half = totrecs / 2;
            DataTable summary1 = panel.Clone();
            DataTable summary2 = panel.Clone();
            int halfR = 0;
            foreach (DataRow dr1 in panel.Rows)
            {
                if (halfR < half)
                {
                    summary1.ImportRow(dr1);
                    halfR++;
                }
                else
                {
                    summary2.ImportRow(dr1);
                    halfR++;
                }
            }
            
            //Networkfname = ProcessVars.SourceDataRoster + "\\Missing_Panel_" + recnum.ToString() + "-1.csv";
            localname = fname + "Missing_Panel_" + recnum.ToString() + "-1.csv";
            resultCSV = createcsv.printCSV_fullProcess(localname, summary1, "", "Y");
            //if (File.Exists(Networkfname))
            //    File.Delete(Networkfname);
            //File.Copy(localname, Networkfname);

            //Networkfname = ProcessVars.SourceDataRoster + "\\Missing_Panel_" + recnum.ToString() + "-2.csv";
            localname = fname + "Missing_Panel_" + recnum.ToString() + "-2.csv";
            resultCSV = createcsv.printCSV_fullProcess(localname, summary2, "", "Y");
            //if (File.Exists(Networkfname))
            //    File.Delete(Networkfname);
            //File.Copy(localname, Networkfname);
