...
namespace Horizon_EOBS_Parse
{
    public class IMBProcess_Back
    {
        ...
        public string update_IMB_back(string todayProcess)
        {
            ...
            DataTable dataFile = dbUIMB.ExecuteDataTable(" select Account_Number as Record_Number, opr_code, [Date] as opr_Date, [Friendly Status] as opr_Status, '' as Source_File, '' as Name, " +
                "'' as Member_ID, '' as Destination_zip, '' as tableName, getdate() as importdate, convert(varchar(20),time) as opr_Time from [IMBDATA_IMPORT_tmp] " +
            ...
            foreach (DataRow row in dataFile.Rows)
            {
                ...
                foreach (DataRow rowC in dataSeqNumbers.Rows)
                {
                        ...
                        ))
                    {
                        ...
                        string strsql = "";
                        if (rowC["Tablename"].ToString() == "HOR_parse_Maintenance_ID_Cards")
                        {
                        ...
                        //    strsql = "select filename,UpdAddr1, '' , UpdZip from HOR_DirectMail where recnum = " + Convert.ToInt32(row["Record_Number"].ToString());
                        
                        //else
                        //    strsql = "select filename,UpdAddr1, memberID, UpdZip from " + rowC["Tablename"] + " where recnum = " + Convert.ToInt32(row["Record_Number"].ToString());
        ...
        public string Update_DirectMail(string todayProcess)
        {
            ...
            GlobalVar.connectionKey = "conStrProd";
            dbU = new DBUtility(GlobalVar.connectionKey, DBUtility.ConnectionStringType.Configured);
            DataTable onlyIdCards = dbU.ExecuteDataTable("SELECT  record_number, opr_code, opr_date, opr_Status  FROM HOR_parse_IMB_received where tablename = 'HOR_DirectMail' " +
