...
namespace Horizon_EOBS_Parse
{
    public class MainProcess
    {
        public void MainProcessParse(string cycleP)
        {
            ...
            string strsql = "delete from HOR_parse_files_to_CASS where msg like 'error count%' and CONVERT(DATE,ImportDate)= '" + GlobalVar.DateofProcess.ToString("yyyy-MM-dd") + "'";
            ...
            foreach (FileInfo file in files)
            {
                ...
                if (ProcessedFName == null)
                {
                    ...
                    if (file.Name.ToUpper().IndexOf("UCDSZ0") == 0)
                    {
                        var msg = "";
                    }
                                            ...
                                            || file.Name.ToUpper().IndexOf("EP006003") == 0)
                    {
                        ...
                        processFiles.processNotices(file.FullName, file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                    }
                    ...
                    if (file.Name.ToUpper().IndexOf("UCDS") == 0)
                    {
                        try
                        {
                            ...
                            if (arrayEOBS.Any(Type.Contains))
                            {
                                ...
                                if (Type == "O")
                                    error = UCDSs.evaluate_EOB_O_TXT(file.FullName, "", file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));

                                else
                                    error = UCDSs.evaluate_EOB_TXT(file.FullName, "", file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                                totfilesPrecessed++;
                                if (error != "")
                                    errors = errors + error + "\n\n";
                                else
                            ...
                            else
                            {
                                ...
                                NParse_UCDS UCDSs = new NParse_UCDS();
                                if (Type == "C")
                                    error = UCDSs.evaluate_EOB_C_TXT(file.FullName, "", file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                                else if (Type == "A")
                                    error = UCDSs.evaluate_EOB_A_TXT(file.FullName, "", file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                                else
                                    error = UCDSs.evaluate_TXT(file.FullName, "", file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                                totfilesPrecessed++;
                                if (error != "")
                                    errors = errors + error + "\n\n";
                                else
                    ...
                    if (file.Name.ToUpper().IndexOf("EPB") == 0)
                    {
                        ...
                        string result = processFiles.Process_Cbills(file.FullName, file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                    }
                    if (file.Name.ToUpper().IndexOf("EP0GH") == 0)
                    {
                        ...
                        string result = processFiles.Process_Gbills(file.FullName, file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                    ...
                    if (file.Name.ToUpper().IndexOf("EPA") == 0 || file.Name.ToUpper().IndexOf("EPM") == 0)
                    {
                        try
                        {
                            ...
                            string error = EPA_EPMs.evaluate_TXT(file.FullName, file.Name.ToUpper().Substring(0, 3), file.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss"));
                            totfilesPrecessed++;
                            if (error != "")
                                errors = errors + error + "\n\n";
                            else
